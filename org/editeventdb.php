...
$eventdat=mysql_query("SELECT * FROM events ");
$qwe=mysql_query("SELECT * FROM organizers WHERE orgid='".mysql_real_escape_string($_SESSION['tz_organizer'])."' ");
if(mysql_num_rows($qwe)<1){echo exit;}
$getuserdata=mysql_fetch_array($qwe);
              ...
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Edit Event to <?php echo $title;?></h4>
                           <?php
                                 $edit_event=mysql_query("SELECT * FROM events where eid='$eid' ");
                                    while($edit=mysql_fetch_array($edit_event))
                                       { 
                                        
                                       
                               ?>
                  <form action="editeventupdate" method="post"  enctype="multipart/form-data">
